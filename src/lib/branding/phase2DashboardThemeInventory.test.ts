...
const DASHBOARD_CONTENT_FILES = [
  ...
  "../../components/conversations/InboxLayout.tsx",
  "../../components/conversations/MessageThread.tsx",
  "../../components/dashboard/A2pStatusCard.tsx",
  "../../components/dashboard/CallForwardingNudge.tsx",
  "../../components/dashboard/DashboardOverview.tsx",
  "../../components/entitlements/FeatureStatusBanners.tsx",
  "../../components/entitlements/LockedFeatureCard.tsx",
  "../../components/knowledge-gaps/KnowledgeGapsDashboard.tsx",
] as const;
...
describe("Phase 2 Diff 7 dashboard theme inventory", () => {
  it.each([
    "#ea580c",
    "#ff914d2e",
    "rgba(234,88,12,.12)",
    "rgb(255 145 77 / .18)",
    "rgb(249 115 22)",
  ])("recognizes direct dashboard brand-color syntax %s", (directColor) => {
    expect(
      DIRECT_BRAND_COLOR.test(directColor) || DIRECT_BRAND_RGB.test(directColor),
    ).toBe(true);
  });

  it.each(DASHBOARD_CONTENT_FILES)(
  ...
  it.each(DASHBOARD_CONTENT_FILES)(
    "does not hardcode ordinary SimplAssist UI copy in %s",
    (relativePath) => {
      const source = readFileSync(new URL(relativePath, import.meta.url), "utf8");
