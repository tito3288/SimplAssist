 ...
 * - identity, and every brand-kind rejection: retry reuses the existing
 *   carrier brand record, so corrected identity details are saved but not
 *   re-filed with the carrier until brand-level recovery ships.
 */
...
export function mapReasonToFriendly(
  ...
  switch (classifyRejection(kind, reason)) {
    ...
    case "url":
      // Brand-kind url fixes are saved but not re-filed with the carrier by
      // retry (same limitation as identity) — don't promise self-serve.
      // Campaign-kind url is about the registration's privacy/terms links:
      // the submitted campaign contains no website URL. Don't point at
      // Settings → Compliance — the dashboard is unreachable while a
      // rejection is showing (dashboardReady is false), so the only
      // self-serve lever is the customer's own site being down; everything
      // else (wrong override value, our hosted pages) is support's.
      return kind === "brand"
        ? `Carriers couldn't verify your website or your privacy and terms pages. Make sure your website address is correct and loads for visitors, then contact support at ${SUPPORT_EMAIL} and we'll re-file your verification with you. (Your SimplAssist privacy and terms pages are generated automatically — if you use your own links, check those.)`
