...
const CONVERTED_FILES = [
  ...
  "./A2pStatusCard.tsx",
  "./CallForwardingNudge.tsx",
  "./DashboardOverview.tsx",
  "../entitlements/FeatureStatusBanners.tsx",
  "../entitlements/LockedFeatureCard.tsx",
] as const;
...
describe("Phase 2 Diff 7 dashboard and billing inventory", () => {
  ...
  it("routes billing plan features through the shared brand presentation", () => {
    const source = readFileSync(
      new URL("../../app/(dashboard)/billing/page.tsx", import.meta.url),
      "utf8"
    );

    expect(source).toContain("getPlanPresentation(key, brand.name)");
    expect(source).toContain("presentedPlan.features.map");
  });
});
