...
import {
  websiteScanIdSchema,
  websiteScanPublishMutationSchema,
...
import { publishWebsiteScanReview } from "@/lib/website-scans/ownerActions.server";
...
export async function POST(
  ...
  const parsed = websiteScanPublishMutationSchema.safeParse(body);
  if (!parsed.success) {
    return websiteScanJson(
  ...
  try {
    ...
    const result = await publishWebsiteScanReview(client, {
    ...
    if (!scan) return websiteScanReadErrorResponse("published scan not readable");
    return websiteScanJson({ scan });
  } catch (error) {
    if (error instanceof WebsiteScanDraftValidationError) {
      return websiteScanJson(
        { error: error.message, code: "invalid_review" },
        { status: 422 },
      );
    }
