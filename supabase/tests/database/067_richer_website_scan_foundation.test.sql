...
INSERT INTO auth.users(id,email) VALUES
 ('00000000-0000-4000-a067-000000000001','scan-owner-a067@example.test'),
 ('00000000-0000-4000-a067-000000000002','other-owner-a067@example.test');
INSERT INTO public.businesses(id,owner_id,name,business_type,slug,website_url) VALUES
 ('10000000-0000-4000-a067-000000000001','00000000-0000-4000-a067-000000000001','Scan A','general','scan-a-067','https://scan-a.example'),
 ('10000000-0000-4000-a067-000000000002','00000000-0000-4000-a067-000000000002','Scan B','general','scan-b-067','https://scan-b.example'),
 ('10000000-0000-4000-a067-000000000003','00000000-0000-4000-a067-000000000001','Lease Scan','general','scan-c-067','https://scan-c.example');
INSERT INTO public.businesses(
  id,owner_id,name,business_type,slug,website_url,onboarding_completed_at
) VALUES (
  '10000000-0000-4000-a067-000000000004','00000000-0000-4000-a067-000000000001',
  'Entitlement Scan','general','scan-d-067','https://scan-d.example',clock_timestamp()
);

INSERT INTO public.services(business_id,name) VALUES
 ('10000000-0000-4000-a067-000000000001','Existing One'),
 ('10000000-0000-4000-a067-000000000001','Existing Two');
INSERT INTO public.faqs(business_id,question,answer) VALUES
 ('10000000-0000-4000-a067-000000000001','Existing question one?','Yes.'),
 ('10000000-0000-4000-a067-000000000001','Existing question two?','Yes.');

CREATE TEMP TABLE scan_067_state(name text PRIMARY KEY,uuid_value uuid,integer_value integer,text_value text);
GRANT SELECT,INSERT,UPDATE ON pg_temp.scan_067_state TO authenticated;
SELECT set_config('request.jwt.claim.sub','00000000-0000-4000-a067-000000000001',true);
SET LOCAL ROLE authenticated;
INSERT INTO scan_067_state(name,uuid_value)
SELECT 'main_scan',(public.start_website_scan_v1(
  '10000000-0000-4000-a067-000000000001','https://scan-a.example','onboarding',
  '20000000-0000-4000-a067-000000000001'
)).id;

SELECT is(
  (public.start_website_scan_v1('10000000-0000-4000-a067-000000000001','https://scan-a.example',
    'onboarding','20000000-0000-4000-a067-000000000001')).id,
  (SELECT uuid_value FROM scan_067_state WHERE name='main_scan'),
  'start is idempotent'
);
SELECT throws_ok(
  $$SELECT public.start_website_scan_v1('10000000-0000-4000-a067-000000000002',
    'https://scan-b.example','onboarding','20000000-0000-4000-a067-000000000002')$$,
  '42501','website_scan_business_not_accessible','an owner cannot start a foreign scan'
);
SELECT throws_ok(
  $$SELECT public.start_website_scan_v1('10000000-0000-4000-a067-000000000004',
    'https://scan-d.example','onboarding','20000000-0000-4000-a067-000000000005')$$,
  '42501','website_scan_purpose_mismatch',
  'a direct RPC cannot start onboarding after onboarding is complete'
);
SELECT throws_ok(
  $$SELECT public.start_website_scan_v1('10000000-0000-4000-a067-000000000004',
    'https://scan-d.example','manual_rescan','20000000-0000-4000-a067-000000000006')$$,
...
INSERT INTO public.subscriptions(
  ...
  '10000000-0000-4000-a067-000000000004','cus_scan_d_a067','sub_scan_d_a067','chat_only','active'
);
SET LOCAL ROLE authenticated;
SELECT lives_ok(
  $$SELECT public.start_website_scan_v1('10000000-0000-4000-a067-000000000004',
...
WHERE business_id='10000000-0000-4000-a067-000000000004';
UPDATE public.subscriptions SET status='canceled'
WHERE business_id='10000000-0000-4000-a067-000000000004';
SET LOCAL ROLE authenticated;
SELECT throws_ok(
  $$SELECT public.save_website_scan_review_v1(
    (SELECT id FROM public.website_scan_runs WHERE business_id='10000000-0000-4000-a067-000000000004'),
    1,'{}'::jsonb)$$,
...
SELECT throws_ok(
  $$SELECT public.publish_website_scan_v1(
    (SELECT id FROM public.website_scan_runs WHERE business_id='10000000-0000-4000-a067-000000000004'),
    1,'30000000-0000-4000-a067-000000000004',
...
SELECT throws_ok(
  $$SELECT public.retry_website_scan_v1(
    (SELECT id FROM public.website_scan_runs WHERE business_id='10000000-0000-4000-a067-000000000004'),
    '30000000-0000-4000-a067-000000000005')$$,
...
UPDATE public.website_scan_runs SET status='published',progress_stage='done',
  published_at=clock_timestamp(),published_idempotency_key='30000000-0000-4000-a067-000000000006'
WHERE business_id='10000000-0000-4000-a067-000000000004';
SET LOCAL ROLE authenticated;
SELECT is(
  public.publish_website_scan_v1(
    (SELECT id FROM public.website_scan_runs WHERE business_id='10000000-0000-4000-a067-000000000004'),
    1,'30000000-0000-4000-a067-000000000006',
    '{"services":[],"faqs":[],"knowledge":[],"questions":[]}'::jsonb
  )->>'status',
...
UPDATE public.website_scan_runs SET status='queued',progress_stage='queued',
  retry_idempotency_key='30000000-0000-4000-a067-000000000007'
WHERE business_id='10000000-0000-4000-a067-000000000004';
SET LOCAL ROLE authenticated;
SELECT is(
  (public.retry_website_scan_v1(
    (SELECT id FROM public.website_scan_runs WHERE business_id='10000000-0000-4000-a067-000000000004'),
    '30000000-0000-4000-a067-000000000007'
  )).status,
...
INSERT INTO scan_067_state(name,uuid_value,integer_value)
SELECT 'claimed',claimed.id,claimed.claim_generation FROM public.claim_next_website_scan_v1('worker-a067',120) claimed;
UPDATE scan_067_state SET text_value=(SELECT claim_token::text FROM public.website_scan_runs
  WHERE id=uuid_value) WHERE name='claimed';
SELECT ok(
  public.update_website_scan_progress_v1(
    (SELECT uuid_value FROM scan_067_state WHERE name='claimed'),
    (SELECT text_value::uuid FROM scan_067_state WHERE name='claimed'),
    (SELECT integer_value FROM scan_067_state WHERE name='claimed'),
    'crawling','fc-job-a067',1,1,0,0,1
  ),
...
SELECT throws_ok(
  format($sql$SELECT public.save_website_scan_page_v1(%L::uuid,%L::uuid,%s,1,
    ...
    (SELECT uuid_value FROM scan_067_state WHERE name='claimed'),
    (SELECT text_value FROM scan_067_state WHERE name='claimed'),
    (SELECT integer_value FROM scan_067_state WHERE name='claimed'),
...
INSERT INTO public.website_scan_runs(
  ...
  '10000000-0000-4000-a067-000000000002','00000000-0000-4000-a067-000000000002',
  'onboarding','https://scan-b.example','20000000-0000-4000-a067-000000000004',
...
SELECT throws_ok(
  format($sql$SELECT public.publish_website_scan_v1(%L::uuid,1,%L::uuid,%L::jsonb)$sql$,
    (SELECT uuid_value FROM scan_067_state WHERE name='main_scan'),
    '30000000-0000-4000-a067-000000000006',
    jsonb_build_object(
...
SELECT lives_ok(
  format($sql$SELECT public.publish_website_scan_v1(%L::uuid,1,%L::uuid,%L::jsonb)$sql$,
    (SELECT uuid_value FROM scan_067_state WHERE name='main_scan'),
    '30000000-0000-4000-a067-000000000001',
    jsonb_build_object(
...
SELECT ok(
  (SELECT count(*)=3 FROM public.services WHERE business_id='10000000-0000-4000-a067-000000000001' AND is_active)
  AND (SELECT count(*)=3 FROM public.faqs WHERE business_id='10000000-0000-4000-a067-000000000001' AND is_active)
  AND EXISTS(SELECT 1 FROM public.business_knowledge_items
    WHERE business_id='10000000-0000-4000-a067-000000000001' AND source='owner_answer'),
...
SELECT ok(
  NOT EXISTS(
    ...
    WHERE scan_id=(SELECT uuid_value FROM scan_067_state WHERE name='main_scan')
...
SELECT ok(
  ...
  AND EXISTS(
    ...
    WHERE business_id='10000000-0000-4000-a067-000000000001'
...
SELECT is(
  (public.publish_website_scan_v1(
    (SELECT uuid_value FROM scan_067_state WHERE name='main_scan'),1,
    '30000000-0000-4000-a067-000000000001','{"services":[],"faqs":[],"knowledge":[],"questions":[]}'::jsonb
...
WHERE id='10000000-0000-4000-a067-000000000001';
INSERT INTO public.subscriptions(
  business_id,stripe_customer_id,stripe_subscription_id,plan,status
) VALUES (
  '10000000-0000-4000-a067-000000000001','cus_scan_a_a067','sub_scan_a_a067','chat_only','active'
);
INSERT INTO public.website_scan_runs(
  ...
  '21000000-0000-4000-a067-000000000001','10000000-0000-4000-a067-000000000001',
  '00000000-0000-4000-a067-000000000001','manual_rescan','https://scan-a.example',
  '22000000-0000-4000-a067-000000000001','ready_for_review','complete','review',1,clock_timestamp()
);
...
SELECT '23000000-0000-4000-a067-000000000001','21000000-0000-4000-a067-000000000001',
...
WHERE s.business_id='10000000-0000-4000-a067-000000000001' AND s.name='Existing One';
INSERT INTO public.website_scan_suggestions(
  id,scan_id,business_id,client_key,kind,category,dedupe_key,draft_payload,
  change_type,target_id,baseline_hash
)
SELECT '23000000-0000-4000-a067-000000000002','21000000-0000-4000-a067-000000000001',
...
SELECT throws_ok(
  format($sql$SELECT public.publish_website_scan_v1(
    '21000000-0000-4000-a067-000000000001',1,'24000000-0000-4000-a067-000000000001',%L::jsonb)$sql$,
    jsonb_build_object(
      'services',jsonb_build_array(jsonb_build_object(
        'suggestionId','23000000-0000-4000-a067-000000000001','targetId',
        (SELECT id FROM public.services WHERE business_id='10000000-0000-4000-a067-000000000001'
          AND name='Existing One'),
        'baselineHash',(SELECT baseline_hash FROM public.website_scan_suggestions
          WHERE id='23000000-0000-4000-a067-000000000001'),
...
UPDATE public.businesses SET onboarding_completed_at=clock_timestamp()
WHERE id='10000000-0000-4000-a067-000000000002';
INSERT INTO public.subscriptions(
  business_id,stripe_customer_id,stripe_subscription_id,plan,status
) VALUES (
  '10000000-0000-4000-a067-000000000002','cus_scan_b_a067','sub_scan_b_a067','chat_only','active'
);
INSERT INTO public.website_scan_runs(
  id,business_id,requested_by,purpose,source_url,idempotency_key,status,coverage,
  ...
  '21000000-0000-4000-a067-000000000002','10000000-0000-4000-a067-000000000002',
  '00000000-0000-4000-a067-000000000002','manual_rescan','https://scan-b.example',
  '22000000-0000-4000-a067-000000000002','failed','insufficient','done','fc-retry-a067',1,3,
...
INSERT INTO public.website_scan_pages(
  ...
  '21000000-0000-4000-a067-000000000002','10000000-0000-4000-a067-000000000002',0,
  'https://scan-b.example/stale','failed',0,'provider_page_failed'
);
SELECT set_config('request.jwt.claim.sub','00000000-0000-4000-a067-000000000002',true);
SET LOCAL ROLE authenticated;
DO $$BEGIN
  PERFORM public.retry_website_scan_v1(
    '21000000-0000-4000-a067-000000000002','24000000-0000-4000-a067-000000000003'
  );
END$$;
RESET ROLE;
SELECT ok(
  EXISTS(
    ...
    WHERE id='21000000-0000-4000-a067-000000000002' AND status='queued'
      AND started_at IS NULL AND provider_job_id='fc-retry-a067' AND provider_job_attempt=0
      ...
      AND NOT EXISTS(
        ...
        WHERE p.scan_id='21000000-0000-4000-a067-000000000002'
      )
  ),
...
VALUES('10000000-0000-4000-a067-000000000003','00000000-0000-4000-a067-000000000001',
  'manual_rescan','https://scan-c.example','20000000-0000-4000-a067-000000000003');
INSERT INTO scan_067_state(name,uuid_value,integer_value,text_value)
...
WHERE id=(SELECT uuid_value FROM scan_067_state WHERE name='lease_one');
INSERT INTO scan_067_state(name,uuid_value,integer_value,text_value)
...
SELECT is(
  public.heartbeat_website_scan_v1(
    (SELECT uuid_value FROM scan_067_state WHERE name='lease_one'),
    (SELECT text_value::uuid FROM scan_067_state WHERE name='lease_one'),
    (SELECT integer_value FROM scan_067_state WHERE name='lease_one'),120),false,
  'the stale claim token and generation cannot heartbeat'
);
SELECT is(
  public.heartbeat_website_scan_v1(
    (SELECT uuid_value FROM scan_067_state WHERE name='lease_two'),
    (SELECT text_value::uuid FROM scan_067_state WHERE name='lease_two'),
    (SELECT integer_value FROM scan_067_state WHERE name='lease_two'),120),true,
...
SELECT lives_ok(
  ...
  BEGIN
    ...
    FROM pg_temp.scan_067_state WHERE name='lease_two';
...
SELECT is(
  (
    ...
    WHERE p.scan_id=(SELECT uuid_value FROM pg_temp.scan_067_state WHERE name='lease_two')
...
SELECT ok(
  public.fail_website_scan_v1(
    (SELECT uuid_value FROM pg_temp.scan_067_state WHERE name='lease_two'),
    (SELECT text_value::uuid FROM pg_temp.scan_067_state WHERE name='lease_two'),
    (SELECT integer_value FROM pg_temp.scan_067_state WHERE name='lease_two'),
...
SELECT is(
  (
    ...
    WHERE page.scan_id=(SELECT uuid_value FROM pg_temp.scan_067_state WHERE name='lease_two')
  ),
...
WHERE id='10000000-0000-4000-a067-000000000001';
SELECT ok(
  NOT EXISTS(SELECT 1 FROM public.website_scan_runs WHERE business_id='10000000-0000-4000-a067-000000000001')
  AND NOT EXISTS(SELECT 1 FROM public.business_knowledge_items WHERE business_id='10000000-0000-4000-a067-000000000001'),
