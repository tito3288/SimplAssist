...
- Non-admins receive 404 (not 403). Amended 2026-07-24 with the dual-session design: signed-out visitors to the `/admin` pages now see an admin login form (a deliberate, accepted trade of page-level stealth for a self-serve sign-in); authenticated non-allowlisted admin-channel sessions are revoked by middleware and see the form again; `/api/admin` routes remain 404-indistinguishable for every non-admin caller. An unset/empty allowlist still 404s everywhere — an unconfigured deploy never renders the form.
- Fail closed: env var unset/empty = nobody is admin.
- Dual-session admin auth (2026-07-24): the admin console authenticates from its own cookie namespace (`sa-admin-auth`, via `@supabase/ssr` `cookieOptions.name`), fully separate from the customer session — an admin session and a customer session coexist in one browser with independent sign-in/sign-out. Same Supabase auth backend, same env allowlist; only session storage is separated. Customer-facing sign-outs keep the global default (adversarial-review decision: it is the customer's only remote-revocation lever for sessions abandoned on other devices, and it also revokes any stray admin-channel session minted with that customer's own credentials). The admin sign-out button uses `scope: "local"` (per-browser). Consequence: the dedicated admin identity must NEVER sign into the customer app — a global customer sign-out as that user would revoke the admin channel too; recovery is simply re-login at `/admin`. The admin cookie sets the Secure attribute in production.
